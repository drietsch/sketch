...
describe('isolation', () => {
  test('editing one node leaves every other group byte-identical', () => {
    ...
    const before = new Map(demo.frameAt().groups.map((g) => [g.key, g.html]));
    demo.scene.update('rounded', { x: 30, cornerRadius: 20 });
    const after = demo.frameAt().groups;
  ...
  test('sketchVariant re-rolls only that node', () => {
    ...
    a.rectangle({ id: 'x', x: 0, y: 0, width: 50, height: 50 });
    a.rectangle({ id: 'y', x: 50, y: 50, width: 50, height: 50 });
    const b = createDemo({ width: 100, height: 100, seed: 5 });
    b.rectangle({ id: 'x', x: 0, y: 0, width: 50, height: 50, sketchVariant: 1 });
    b.rectangle({ id: 'y', x: 50, y: 50, width: 50, height: 50 });
    const ga = a.frameAt().groups;
    const gb = b.frameAt().groups;
  ...
  test('a node moved by its parent keeps its geometry and changes only its transform', () => {
    ...
    demo.rectangle({ id: 'p', x: 0, y: 0, width: 100, height: 100 });
    demo.rectangle({ id: 'c', parent: 'p', x: 10, y: 10, width: 20, height: 20 });
    const before = demo.frameAt().groups[1].html;
    demo.scene.update('p', { x: 50 });
    const after = demo.frameAt().groups[1].html;
