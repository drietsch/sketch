...
describe('removing through the scene', () => {
  test('frees the cached geometry of the node and its descendants', () => {
    ...
    demo.frame({ id: 'p', x: 0, y: 0, width: 100, height: 100 });
    demo.button({ id: 'b', parent: 'p', x: 10, y: 10, characters: 'Go' });
    demo.rectangle({ id: 'keep', x: 150, y: 150, width: 10, height: 10 });
    demo.toSVG();
  ...
  test('the hook fires once with the whole subtree, in removal order', () => {
    ...
    scene.add({ id: 'b', type: 'RECTANGLE', x: 0, y: 0, width: 1, height: 1 });
    scene.remove('a');
  ...
  test("removing during playback does not disturb the compiled timeline's working copy", () => {
    const demo = createDemo({ width: 200, height: 200, seed: 1 });
    demo.button({ id: 'go', x: 10, y: 10, characters: 'Go' });
    demo.rectangle({ id: 'extra', x: 100, y: 100, width: 10, height: 10 });
